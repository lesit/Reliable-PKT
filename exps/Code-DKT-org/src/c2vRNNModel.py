...
class c2vRNNModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, layer_dim, output_dim, node_count, path_count, device, org_att_dim, dkt):
        ...
        if dkt:
            ...
            self.attention_softmax = nn.Softmax(dim = (1 if org_att_dim else 2))
            self.dropout = nn.Dropout(p=0.1)

            rnn_input_size = 2*input_dim+300
