...
class data_reader():
    def __init__(self, data_dir, train_path, val_path, test_path, maxstep, numofques, dkt:bool):
    ...
    def get_data(self, file_path):
        ...
        if not self.dkt:
            code_df = pd.read_csv(f"{self.data_dir}/labeled_paths.tsv",sep="\t")
            training_students = np.load(f"{self.data_dir}/training_students.npy",allow_pickle=True)
            ...
            np.save(f"{self.data_dir}/np_counts.npy", [node_count, path_count])
        ...
        with open(file_path, 'r') as file:
            for lent, css, ques, ans in itertools.zip_longest(*[file] * 4):
                ...
                css = [int(cs) for cs in css.strip().strip(',').split(',')]
