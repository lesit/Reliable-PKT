...
def main():
    logger = make_logger.make("path_extractor", save_dir="./")
    logger.info("start")
    
    config = Config()

    code_df = pd.read_csv("../../../dataset/CodeWorkout/LinkTables/CodeStates.csv")
    ...
    for assignment_id in assignment_list:
        ...
        save_path = os.path.join(save_dir, f"labeled_paths.tsv")
        ...
        parsed_code = []
        for c in tqdm.tqdm(list(assignment_df['Code'])):
            try:
        ...
        AST_paths = []
        for java_code in tqdm.tqdm(parsed_code):
            ast_path = extracting_path(java_code, max_length=config.code_path_length, max_width=config.code_path_width, hash_path=True, hashing_table=hashing_table)
